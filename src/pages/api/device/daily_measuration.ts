...
// const calculateValues = (wunderData: any, ecowittData: any) => {
//   // Extract Wunder data
...
// // Extract Ecowitt data if available (e.g., lightning, pm25)
// const lightningCount = ecowittData?.data?.lightning?.count?.list ? Math.max(...Object.values(ecowittData.data.lightning.count.list).map(Number)) : 0;

// // Check if pm25.list is an array before performing calculations
...
//     // Convert pm25list object into an array of PM2.5 values (convert string to number)
...
//     humidityAvg,
//     lightningCount,
//     pm25Avg: pm25Stats.min,
//     pm25Max: pm25Stats.max,
//     pm25Min: pm25Stats.avg,
//   };
// };
...
const getYesterdayDate = () => {
  ...
  const formattedDate = yesterday.toISOString().split('T')[0].replace(/-/g, ''); // Format as 'YYYYMMDD'

  // Ensure that the date is in the correct format (YYYYMMDD)
  if (!/^\d{8}$/.test(formattedDate)) {
    throw new Error(`Invalid yesterday date string: ${formattedDate}`);
  }

  // Convert the formattedDate back into a Date object to ensure validity
  const year = parseInt(formattedDate.substring(0, 4), 10);
  const month = parseInt(formattedDate.substring(4, 6), 10) - 1; // JavaScript months are 0-indexed
